 ...
 */
const FS_PREFIX = `/@fs/`;
const WITE_PACKAGE_DIR = path$3.resolve(
...
const CLIENT_ENTRY = path$3.resolve(WITE_PACKAGE_DIR, 'dist/client/client.mjs');
path$3.resolve(WITE_PACKAGE_DIR, 'dist/client/env.mjs');
...
// set in bin/wite.js
const filter = process.env.WITE_DEBUG_FILTER;
...
// Use splitVendorChunkPlugin() to get the same manualChunks strategy as Wite 2.7
// We don't recommend using this strategy as a general solution moving forward
// splitVendorChunk is a simple index/vendor strategy that was used in Wite
...
function splitVendorChunkPlugin() {
    ...
    return {
        name: 'wite:split-vendor-chunk',
        config(config) {
            let outputs = config?.build?.rollupOptions?.output;
            if (outputs) {
                outputs = Array.isArray(outputs) ? outputs : [outputs];
                for (const output of outputs) {
                    const witeManualChunks = createSplitVendorChunk(output, config);
                    if (witeManualChunks) {
                        if (output.manualChunks) {
                            if (typeof output.manualChunks === 'function') {
                                const userManualChunks = output.manualChunks;
                                output.manualChunks = (id, api) => {
                                    return userManualChunks(id, api) ?? witeManualChunks(id, api);
                                };
                            }
                            else {
                            ...
                            output.manualChunks = witeManualChunks;
                        }
                    }
                }
            }
...
const debug = createDebugger('wite:sourcemap', {
...
function createLogger(level = 'info', options = {}) {
    ...
    const { prefix = '[wite]', allowClearScreen = true } = options;
...
function loadEnv(mode, envDir, prefixes = 'WITE_') {
    if (mode === 'local') {
    ...
    if (parsed.NODE_ENV && process.env.WITE_USER_NODE_ENV === undefined) {
        process.env.WITE_USER_NODE_ENV = parsed.NODE_ENV;
    }
    ...
    // check if there are actual env variables starting with WITE_*
...
function resolveEnvPrefix({ envPrefix = 'WITE_', }) {
