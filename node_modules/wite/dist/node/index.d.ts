...
export declare interface BuildOptions {
     ...
     * instead of using the one generated by Wite.
     *
     * Example:
     *
     * ```json
...
export declare interface CommonServerOptions {
    /**
     * Specify server port. Note if the port is already being used, Wite will
     * automatically try the next available port so this may not be the actual
     * port the server ends up listening on.
     */
    port?: number;
    /**
     * If enabled, wite will exit if specified port is already in use
     */
    strictPort?: boolean;
    /**
...
export declare function createServer(inlineConfig?: InlineConfig): Promise<WiteDevServer>;
...
/**
 * Type helper to make it easier to use wite.config.ts
...
export declare interface DepOptimizationConfig {
     ...
     * with Wite's dep optimization.
     *
     * - `external` is also omitted, use Wite's `optimizeDeps.exclude` option
     * - `plugins` are merged with Wite's dep plugin
     *
     ...
     *
     * By default, Wite can optimize `.mjs`, `.js`, `.ts`, and `.mts` files. This option
...
export declare type DepOptimizationOptions = DepOptimizationConfig & {
    /**
     * By default, Wite will crawl your `index.html` to detect dependencies that
     * need to be pre-bundled. If `build.rollupOptions.input` is specified, Wite
     * will crawl those entry points instead.
     *
     * If neither of these fit your needs, you can specify custom entries using
     * this option - the value should be a fast-glob pattern or array of patterns
     * (https://github.com/mrmlnc/fast-glob#basic-syntax) that are relative from
     * wite project root. This will overwrite default entries inference.
     */
...
export declare interface ExperimentalOptions {
    ...
    /**
     * Skips SSR transform to make it easier to use Wite with Node ESM loaders.
     * @warning Enabling this will break normal operation of Wite's SSR in development mode.
     *
     * @experimental
     * @default false
     */
 ...
 */
export declare function isFileServingAllowed(url: string, server: WiteDevServer): boolean;
...
export declare interface LegacyOptions {
    /**
     * Revert wite build --ssr to the v2.9 strategy. Use CJS SSR build and v2.9 externalization heuristics
     *
     * @experimental
     * @deprecated
     * @default false
 ...
 * Used by Wite CLI when running `wite optimize`.
 */
...
/**
 * Wite plugins extends the Rollup plugin interface with a few extra
 * wite-specific options. A valid wite plugin is also a valid Rollup plugin.
 * On the contrary, a Rollup plugin may or may NOT be a valid wite universal
...
declare interface Plugin_2 extends Plugin_3 {
    ...
    /**
     * Modify wite config before it's resolved. The hook can either mutate the
    ...
    /**
     * Use this hook to read and store the final resolved wite config.
     */
    configResolved?: ObjectHook<(this: void, config: ResolvedConfig) => void | Promise<void>>;
    /**
     * Configure the wite server. The hook receives the {@link WiteDevServer}
     ...
     *
     * - html: string
     * - ctx?: wite.ServerContext (only present during serve)
     * - bundle?: rollup.OutputBundle (only present during build)
     *
     * It can either return a transformed string, or a list of html tag
     * descriptors that will be injected into the `<head>` or `<body>`.
     *
     * By default the transform is applied **after** wite's internal html
     * transform. If you need to apply the transform before wite, use an object:
     ...
     *   e.g. for a Kdu SFC, we can narrow down the part to update by comparing
     *   the descriptors.
     *
...
/**
 * Starts the Wite server in preview mode, to simulate a production deployment
 */
...
export declare interface PreviewServerForHook {
    ...
    /**
     * The resolved urls Wite prints on the CLI
     */
...
/**
 * Resolve base url. Note that some users use Wite to build for non-web targets like
...
export declare type ServerHook = (this: void, server: WiteDevServer) => (() => void) | void | Promise<(() => void) | void>;
...
export declare interface ServerOptions extends CommonServerOptions {
    ...
    watch?: WatchOptions;
    /**
     * Create Wite dev server to be used as a middleware in an existing server
     * @default false
     */
...
export declare interface SSROptions {
    ...
    target?: SSRTarget;
    /**
     * Define the format for the ssr build. Since Wite v3 the SSR build generates ESM by default.
...
export declare interface UserConfig {
     ...
     * deps or some other cache files that generated by wite, which can improve
     * the performance. You can use `--force` flag or manually delete the directory
     * to regenerate the cache files. The value can be either an absolute file
     * system path or a path relative to project root.
     * Default to `.wite` when no `package.json` is detected.
     * @default 'node_modules/.wite'
     */
    cacheDir?: string;
    /**
     ...
     * Please be careful and always pin Wite's version when using them.
     * @experimental
     */
    experimental?: ExperimentalOptions;
    /**
     ...
     * future minor version. Please always pin Wite's version to a minor when using them.
     */
    ...
    worker?: {
        ...
        format?: 'es' | 'iife';
        /**
         * Wite plugins that apply to worker bundle
         */
        plugins?: PluginOption[];
        /**
...
export declare interface WiteDevServer {
    /**
     * The resolved wite config object
     */
    config: ResolvedConfig;
    ...
    /**
     * The resolved urls Wite prints on the CLI. null in middleware mode or
